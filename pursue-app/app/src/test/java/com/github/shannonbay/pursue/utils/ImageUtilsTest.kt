...
class ImageUtilsTest {
    ...
    fun `test createLetterAvatar uses default color`() {
        // Given
        val name = "Test User"
        val defaultColor = android.graphics.Color.parseColor("#1976D2")
    ...
    fun `test createLetterAvatar uses custom color`() {
        // Given
        val name = "Test User"
        val customColor = android.graphics.Color.parseColor("#FF5722") // Deep Orange
    ...
    fun `test uriToFile handles IOException`() {
        ...
        val mockUri = mockk<Uri>(relaxed = true)
        val mockContentResolver = mockk<ContentResolver>(relaxed = true)
        every { mockContentResolver.openInputStream(mockUri) } throws java.io.IOException("Test error")
