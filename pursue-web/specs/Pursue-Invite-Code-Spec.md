...
```json
{
  ...
  "qr_code_url": "https://api.getpursue.app/qr/PURSUE-A7X9K2-M4P8L6",
...
async function getGroupInviteCode(groupId: string, userId: string) {
  ...
  return {
    ...
    qr_code_url: `https://api.getpursue.app/qr/${invite.code}`,
...
```json
{
  ...
  "qr_code_url": "https://api.getpursue.app/qr/PURSUE-B3N7Q9-R5T2W8",
...
**Purpose**: Join a group using an invite code
...
async function joinGroupViaInvite(inviteCode: string, userId: string) {
  ...
  // 3. Check resource limits (max 100 groups per user)
  ...
  return await db.transaction().execute(async (trx) => {
    ...
    await trx
      ...
      .values({
        ...
        metadata: { via_invite_code: inviteCode }
      })
      .execute();
    ...
    const group = await trx
      ...
      .select([
        ...
        'groups.icon_emoji',
        'groups.icon_color',
        trx.fn.count('group_memberships.id').as('member_count')
      ])
      .where('groups.id', '=', invite.group_id)
      .groupBy('groups.id')
      .executeTakeFirst();
    
    // 7. Send FCM notifications to all group members
    await sendGroupNotification(invite.group_id, {
      title: `${await getUserDisplayName(userId)} joined the group`,
      body: `Welcome to ${invite.name}!`,
...
**Endpoint**: `GET /api/qr/:invite_code`

**Purpose**: Generate QR code image for invite code
...
app.get('/api/qr/:invite_code', async (req, res) => {
  ...
  // Generate QR code
  const url = `https://getpursue.app/join/${invite_code}`;
  const qrBuffer = await QRCode.toBuffer(url, {
    width: 256,
    margin: 2,
    color: {
      dark: '#1976D2',  // Pursue blue
      light: '#FFFFFF'
    }
  });
...
- ✅ Joining via invite code adds user to group
...
- ✅ User shares invite link → other user joins successfully
- ✅ Admin regenerates code → old code stops working
- ✅ QR code generation works for valid codes
- ✅ FCM notifications sent when new member joins
