# Pursue â€” Challenges Feature Specification
...
Challenges are built on top of the existing group infrastructure â€” a challenge **is** a group with additional metadata (start/end dates, template source, challenge-specific rules). This means all existing features â€” nudges, reactions, group heat, smart reminders, activity feed, progress logging â€” work inside challenges with zero additional effort.
...
- Challenge cards are inherently shareable â€” every share is a targeted ad to the sharer's social circle
...
**For engagement:**
- Defined end dates create urgency ("Day 22 of 30 â€” don't stop now!")
...
- **Zero new UI paradigms:** Challenges use the existing group detail screen with a challenge header overlay â€” not a separate experience
...
- **Freemium-friendly:** Challenges are the single best viral acquisition channel â€” restricting creation to premium would be leaving growth on the table
...
- Create custom challenges (not from templates) â€” see Â§2.2
...
Free users can only create challenges **from the template library** (Â§4). Premium users can create **custom challenges** with arbitrary goals, durations, and descriptions. This is a meaningful differentiator that keeps the free experience curated and on-brand while giving premium users creative freedom.
...
ALTER TABLE groups ADD COLUMN template_id UUID;       -- NULL for custom challenges
...
CREATE INDEX idx_groups_template 
...
| `template_id` | UUID | FK to `group_templates`; NULL for custom challenges |
| `challenge_status` | varchar(20) | Lifecycle state: `upcoming`, `active`, `completed`, `cancelled` |

**Status transitions:**
```
upcoming â†’ active       (automated: when current date >= challenge_start_date)
upcoming â†’ cancelled    (manual: creator cancels before start)
active â†’ completed      (automated: when current date > challenge_end_date)
active â†’ cancelled      (manual: creator cancels mid-challenge)
...
**"Active" means:** `challenge_status IN ('upcoming', 'active')`. Completed and cancelled challenges do not count toward limits. This means free users can run challenge after challenge sequentially â€” they just can't have two running simultaneously.
...
-- Add FK from groups to group_templates (after template table exists)
...
-- Seed challenge templates (see Â§4 for full list)
...
| **Plank Challenge** | 30 days | Hold plank (duration, target increases weekly: 30sâ†’60sâ†’90sâ†’120s) | Moderate |
...
```typescript
const GROUP_TEMPLATES: GroupTemplateSeed[] = [
  {
...
**Notes:**
- No authentication required â€” template browsing is public (useful for pre-signup marketing page)
...
```json
{
  ...
  "group_name": "No Sugar March ðŸ¬"
}
```

**Request (custom â€” premium only):**
```json
{
  "start_date": "2026-03-01",
  "end_date": "2026-03-31",
...
### 5.3 POST /api/groups/join

Join a challenge via invite code. Uses the existing `/api/groups/join` endpoint â€” no new endpoint needed. The existing join logic checks group limits; the resource limit checker needs to be updated to distinguish challenge limits from regular group limits.
...
### 5.5 PATCH /api/groups/:id/cancel
...
5. Insert activity entry: `group_challenge_cancelled`
...
**Timezone consideration:** The scheduled job runs on UTC. A user in NZ (UTC+12) might see their challenge marked as completed a few hours before midnight local time, or a few hours after. This is acceptable for a daily-granularity feature â€” the same approach used for group heat recalculation. If this becomes an issue, we can shift the job to run at multiple times (e.g., every 6 hours) or use the creator's timezone as the reference.

### 6.2 Challenge Completion Flow

When a challenge transitions to `completed`:

1. **Activity entry:** Insert `group_challenge_completed` activity in the group feed
2. **Milestone notification:** For each member, create a `milestone_achieved` notification with `metadata: { milestone_type: 'group_challenge_completed', challenge_name: '...', completion_rate: 0.93 }`
3. **Shareable challenge card:** Generate `shareable_card_data` on the milestone notification (see Â§8)
4. **FCM push:** Send to all members: "ðŸŽ‰ Challenge Complete! You finished the 30-Day No Sugar Challenge with a 93% completion rate. Tap to share!"
...
- Group heat calculations already handle mid-window joins (per group-heat-spec.md Â§2.2)

### 6.4 Challenge Goals Are Immutable

Unlike regular groups where admins can add/archive goals at any time, challenge goals are locked once the challenge starts (status = `active`). This ensures fairness and consistency â€” everyone in the challenge is working toward the same goals. The creator can still add goals while the challenge is `upcoming`.
...
â”‚  â”‚ ðŸ¬              â”‚ â”‚ ðŸ§˜              â”‚   â”‚
â”‚  â”‚ 30-Day No Sugar â”‚ â”‚ 21-Day          â”‚   â”‚
â”‚  â”‚                 â”‚ â”‚ Meditation      â”‚   â”‚
â”‚  â”‚ 30 days Â· Hard  â”‚ â”‚ 21 days Â· Easy  â”‚   â”‚
...
â”‚  â”‚ 10K Steps Daily â”‚ â”‚ 100 Pushups     â”‚   â”‚
â”‚  â”‚                 â”‚ â”‚ a Day           â”‚   â”‚
â”‚  â”‚ 30 days Â· Med   â”‚ â”‚ 30 days Â· Hard  â”‚   â”‚
â”‚  â”‚ [Start]         â”‚ â”‚ [Start]         â”‚   â”‚
â”‚  â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜ â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜   â”‚
â”‚                                             â”‚
â”‚  ðŸ“š Reading                                 â”‚
...
â”‚  ðŸ”“ Premium: Create custom challenge        â”‚
â”‚  Design your own challenge with any goals.  â”‚
â”‚  [Create Custom]                            â”‚
â”‚  â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€  â”‚
â”‚                                             â”‚
...
- Category filter chips at top (optional, may add visual clutter â€” test without first)
...
```
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚  â† Set Up Challenge                         â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚                                             â”‚
â”‚  ðŸ¬ 30-Day No Sugar Challenge               â”‚
â”‚  Cut out added sugar for 30 days. Track     â”‚
â”‚  your progress daily and hold each other    â”‚
â”‚  accountable.                               â”‚
â”‚                                             â”‚
â”‚  â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”    â”‚
â”‚  â”‚ Challenge Name                      â”‚    â”‚
â”‚  â”‚ No Sugar March ðŸ¬                   â”‚    â”‚
â”‚  â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜    â”‚
â”‚                                             â”‚
â”‚  â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”    â”‚
â”‚  â”‚ Start Date                          â”‚    â”‚
â”‚  â”‚ March 1, 2026                   ðŸ“…  â”‚    â”‚
â”‚  â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜    â”‚
â”‚                                             â”‚
â”‚  End Date: March 30, 2026 (30 days)         â”‚
â”‚                                             â”‚
â”‚  Goals included:                            â”‚
â”‚  âœ… No added sugar today (daily, binary)    â”‚
â”‚                                             â”‚
â”‚                                             â”‚
â”‚  â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”    â”‚
â”‚  â”‚       ðŸš€ Start Challenge            â”‚    â”‚
â”‚  â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜    â”‚
â”‚                                             â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
```
...
â”‚  â”‚  ðŸ¬ 30-Day No Sugar Challenge          â”‚â”‚
â”‚  â”‚  Day 18 of 30                          â”‚â”‚
â”‚  â”‚  â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–‘â–‘â–‘â–‘â–‘â–‘â–‘  60%      â”‚â”‚
...
â”‚                                             â”‚
â”‚  Goals                                      â”‚
â”‚  ... (regular goal list with member         â”‚
â”‚       progress, same as any group)          â”‚
â”‚                                             â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
```
...
- "Share Challenge" button â†’ generates shareable challenge card (Â§8)
- "Invite Friends" button â†’ shows invite code/link with share sheet

**Completed challenge header:**
```
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚  ðŸŽ‰ Challenge Complete!                     â”‚
â”‚  You completed 28 of 30 days (93%)          â”‚
...
```
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚  ðŸƒ Morning Runners          ðŸ”¥ðŸ”¥ðŸ”¥       â”‚ â† Regular group
â”‚  5 members                                  â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚  ðŸ¬ No Sugar March           Day 18/30     â”‚ â† Challenge
â”‚  5 members Â· 12 days left    â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–‘â–‘â–‘     â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚  ðŸ“– Bible in a Year          Day 45/365    â”‚ â† Challenge
â”‚  3 members Â· 320 days left   â–ˆâ–‘â–‘â–‘â–‘â–‘â–‘â–‘     â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
```
...
- Completed challenges show a âœ… badge and greyed progress bar

### 7.5 Challenges in the Today Screen

Challenge goals appear in the Today screen alongside regular group goals, using the existing combined goal list. No special treatment needed â€” they're just goals. The group name badge on each goal row already provides context.
...
Speed Dial Options:
  ...
  2. Start a Challenge (new) â†’ opens template browser
...
**Card layout (1080Ã—1920 for Instagram Stories):**
...
â”‚        Challenge                        â”‚
â”‚                                         â”‚
â”‚     March 1 â€“ March 30, 2026           â”‚
â”‚                                         â”‚
â”‚     "Are you in?"                       â”‚
â”‚                                         â”‚
â”‚     â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”       â”‚
â”‚     â”‚  Join at getpursue.app    â”‚       â”‚
â”‚     â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜       â”‚
â”‚                                         â”‚
â”‚              pursue                     â”‚
â”‚     â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€           â”‚
â”‚     Group accountability for goals      â”‚
â”‚                                         â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
```
...
```json
{
  "card_type": "group_challenge_invite",
  "title": "30-Day No Sugar Challenge",
  "subtitle": "March 1 â€“ March 30, 2026",
  "icon_emoji": "ðŸ¬",
  "cta_text": "Are you in?",
...
```json
{
  "milestone_type": "group_challenge_completed",
...
```
https://getpursue.app/challenge/{invite_code}?utm_source=share&utm_medium=group_challenge_card&utm_campaign={template_slug}&utm_content={user_id}
```
...
**Tap behavior:** Opens the template browser (Â§7.1).
...
| Trigger | Title | Body |
|---------|-------|------|
| 1 day before start | "ðŸ Challenge starts tomorrow!" | "No Sugar March kicks off tomorrow. Make sure your group is ready!" |
| Day 1 | "ðŸš€ Day 1 â€” let's go!" | "Your 30-Day No Sugar Challenge starts today. Log your first entry!" |
| Halfway point | "ðŸ”¥ Halfway there!" | "Day 15 of 30 â€” you're halfway through No Sugar March! Keep pushing." |
| 3 days remaining | "â° 3 days left!" | "Just 3 more days in your No Sugar challenge. Don't stop now!" |
| Final day | "ðŸ† Last day!" | "It's the final day of No Sugar March. Finish strong!" |
| Completed | "ðŸŽ‰ Challenge Complete!" | "You finished the 30-Day No Sugar Challenge! Tap to share your achievement." |

**Implementation:** These are generated by the challenge status update scheduled job (Â§6.1). The job checks for challenges hitting these milestones and queues FCM pushes.
...
|------|--------|--------|-------------|
| Challenge suggestion | ðŸ† system icon | None | None |
| Challenge starts tomorrow | Group icon | Blue left border | ðŸ |
| Challenge completed | Group icon | Gold left border | ðŸŽ‰ |
...
|-------|------|-----------|
| `template_browser_opened` | User opens template browser | â€” |
| `template_viewed` | User taps a template card | `template_slug`, `category` |
| `group_challenge_created` | Challenge created successfully | `template_slug`, `duration_days`, `is_custom` |
| `group_challenge_joined` | User joins a challenge | `challenge_id`, `template_slug`, `days_remaining` |
| `group_challenge_invite_shared` | Challenge invite card shared | `challenge_id`, `share_method` (instagram/generic) |
| `group_challenge_card_viewed` | User opens challenge invite card | `challenge_id` |
| `group_challenge_completed_card_shared` | Completion card shared | `challenge_id`, `completion_rate` |
| `group_challenge_cancelled` | Creator cancels challenge | `challenge_id`, `days_elapsed` |
| `group_suggestion_sent` | Suggestion notification sent | `user_id` |
| `group_suggestion_tapped` | User taps suggestion notification | `user_id` |
| `group_suggestion_converted` | User creates/joins challenge after suggestion | `user_id`, `days_since_suggestion` |
...
|--------|--------|-------------|
| **Challenge creation rate** | >20% of active users create a challenge | `group_challenge_created` / MAU |
| **Template usage** | >80% of challenges use a template | challenges with `template_id` / total challenges |
| **Invite share rate** | >40% of challenge creators share invite | `group_challenge_invite_shared` / `challenge_created` |
| **Join conversion** | >50% of invite recipients join | `group_challenge_joined` / invite link clicks |
...
- [ ] Create `group_template_goals` table
- [ ] Create `group_suggestion_log` table
- [ ] Write migration script
- [ ] Seed initial template data (Â§4)

**API Endpoints:**
- [ ] `GET /api/group-templates` â€” list templates with goals
- [ ] `POST /api/groups` â€” create challenge from template or custom
- [ ] `GET /api/groups` â€” list user's challenges with progress stats
- [ ] `PATCH /api/groups/:id/cancel` â€” cancel challenge
- [ ] Update `POST /api/groups/join` â€” handle challenge-specific limits
- [ ] Update `POST /api/groups/:id/goals` â€” block goal creation on active challenges
- [ ] Update resource limit checks to separate challenge vs regular group limits

**Scheduled Jobs:**
- [ ] Challenge status transition job (upcomingâ†’active, activeâ†’completed)
...
**Notifications:**
- [ ] `group_challenge_completed` milestone notification with shareable card data
...
**Template Browser:**
- [ ] `GroupTemplatesScreen` composable with category rows
- [ ] `GroupTemplateCard` composable (icon, title, duration, difficulty badge)
- [ ] `GroupTemplateViewModel` â€” fetch templates, handle create
- [ ] Category horizontal scroll rows (`LazyRow`)
- [ ] Featured section with larger cards

**Challenge Setup:**
- [ ] `GroupChallengeSetupScreen` composable
...
- [ ] Completed challenge badge (âœ…)

**Speed Dial:**
- [ ] Add "Start a Challenge" option to Speed Dial FAB

**Shareable Cards:**
- [ ] Challenge invite card generation (1080Ã—1920)
...
**Navigation:**
- [ ] Template browser â†’ Setup screen â†’ Group detail
- [ ] Challenge suggestion notification tap â†’ template browser
- [ ] Challenge countdown notification tap â†’ group detail
- [ ] Challenge completion notification tap â†’ shareable card screen
...
- [ ] Challenge lifecycle: upcoming â†’ active â†’ completed
...
6. **Notifications for upcoming challenges:** If a challenge is created with a start date 2 weeks away, should members receive periodic reminders before it starts?
   - **Recommendation:** Just the "1 day before start" notification (Â§9.2). More frequent reminders before the start would feel spammy for something the user voluntarily joined.

7. **Template versioning:** If a template's goals or description are updated, do existing challenges using that template change?
   - **Recommendation:** No. Challenge goals are copied from the template at creation time (snapshot). Template updates only affect future challenges. The `template_id` FK is for analytics/attribution, not live data binding.
